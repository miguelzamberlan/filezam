...
// GetUploadByTarget fetches the session for a destination.
func (db *DB) GetUploadByTarget(ctx context.Context, dir, name string) (*Upload, error) {
	return scanUpload(db.r.QueryRowContext(ctx, `SELECT `+uploadCols+` FROM uploads WHERE dir=? AND name=?`, dir, name))
}
