...
type Config struct {
	...
	UploadReserve  int64         // bytes que as sessões chunked abertas de um usuário podem reservar; 0 = sem teto
...
func Load() (*Config, error) {
	...
	if env("FILEZAM_UPLOAD_MAX_RESERVED", "") == "0" {
		c.UploadReserve = 0
	} else if c.UploadReserve, err = bytesEnv("FILEZAM_UPLOAD_MAX_RESERVED", 100<<30); err != nil {
		return nil, err
	}
...
func bytesEnv(key string, def int64) (int64, error) {
	...
	for _, s := range []struct {
		suf string
		m   int64
	}{{"TIB", 1 << 40}, {"TB", 1 << 40}, {"T", 1 << 40}, {"GIB", 1 << 30}, {"GB", 1 << 30}, {"G", 1 << 30}, {"MIB", 1 << 20}, {"MB", 1 << 20}, {"M", 1 << 20}, {"KIB", 1 << 10}, {"KB", 1 << 10}, {"K", 1 << 10}} {
