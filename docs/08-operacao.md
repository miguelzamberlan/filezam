...
| Pesquisa não acha arquivo copiado por Samba/SSH | o índice só vê mudanças feitas pelo Filezam até a próxima varredura | Aguardar a próxima varredura (intervalo em **Configurações do sistema**) ou **Varrer agora** em **Configurações do sistema** (também **Reconstruir índice** na tela Pesquisar), só admin |
