...
**Traefik v3** na mesma rede Docker: remova o bloco `ports` do compose, descomente `networks`/`labels` no `docker-compose.yml`, coloque a sub-rede Docker em `FILEZAM_TRUSTED_PROXIES` (ex.: `172.16.0.0/12`) e aumente `entryPoints.websecure.transport.respondingTimeouts.readTimeout` (padrão 60 s) para `10m`.

**Cloudflare Tunnel** (`cloudflared` na mesma rede Docker ou no host): aponte o túnel para `http://filezam:8080` (ou `http://127.0.0.1:8080`) e liste a origem do túnel em `FILEZAM_TRUSTED_PROXIES` (o container do `cloudflared`, ou `127.0.0.1`). O proxy da Cloudflare limita cada requisição a 100 MB: mantenha o chunk em até 64 MiB (o padrão de 16 MiB já serve).
...
- Com o proxy na mesma rede Docker, não publique porta: aponte-o para `http://filezam:8080` e coloque a sub-rede em `FILEZAM_TRUSTED_PROXIES`.
...
| `FILEZAM_TRUSTED_PROXIES` | vazio | IPs/CIDRs dos proxies, separados por vírgula. Só deles `X-Forwarded-*` é aceito |
...
Só no `.env` do compose (não são lidas pelo binário): `PUID`/`PGID`, `FILEZAM_HOST_ROOT`, `FILEZAM_HOST_CONFIG`, `FILEZAM_BIND`, `FILEZAM_PORT`, `FILEZAM_VERSION`.
...
- Links públicos ficam amarrados ao inode do item: em sistemas que não informam inode estável (alguns FUSE/SMB) vale só o caminho.
...
| Auditoria mostra sempre o IP do proxy | proxy fora de `FILEZAM_TRUSTED_PROXIES` | Listar o IP/CIDR do proxy |
...
| Link público sempre 404 | expirado, revogado, item movido/renomeado/recriado, dono desativado ou escopo do dono estreitado | Criar novo link / reativar o usuário |
