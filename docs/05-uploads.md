...
- Confere o teto de reserva: a soma dos `size` das sessões abertas do usuário mais o novo não pode passar de `FILEZAM_UPLOAD_MAX_RESERVED` (padrão 100 GiB; `0` = sem teto) → 413 `upload_reserve_exceeded`. A pré-alocação ocupa o disco antes de o primeiro byte chegar; sem o teto, um usuário sem cota reservaria o disco inteiro por até 24 h. Cancelar ou concluir uma sessão libera a reserva.
- Insere a linha em `uploads` (índice único `(user_id, dir, name)` base-relativo, migração 010 → 409 `upload_in_progress` só para o **mesmo usuário**). Outro usuário do mesmo escopo pode ter a sua sessão para o mesmo destino: ninguém bloqueia nem cancela o envio alheio, e quem finaliza primeiro grava (o outro recebe 409 `exists` no `complete`, como se o arquivo tivesse surgido no meio tempo).
- **Sessão interrompida do mesmo dono cede o lugar** (`Service.supersede`): se a sessão aberta naquele destino é de um arquivo comprovadamente diferente — outro `size`, ou `mtime` afastado mais de 1 s —, ela é apagada com a sua parte e a nova entra no lugar. Sem isso, reenviar o mesmo vídeo em outra qualidade (outro tamanho, e por isso impossível de retomar) falhava com `upload_in_progress` até a limpeza das 24 h, e a única saída na interface era descartar **todas** as pendências. Duas travas: a sessão precisa ser do mesmo dono e, no modo drop, do mesmo remetente; e nenhum bloco pode estar sendo gravado nela naquele instante (`writing`, contado em `WriteChunk`) — quem está enviando agora tem sempre um `PUT` em voo e não perde o que já subiu para outra aba. Sem `mtime` dos dois lados, ou com `size` e `mtime` iguais, nada é descartado: isso é retomada, e quem adota a sessão é o cliente.
- A conferência do teto e a inserção acontecem sob um mutex (`Service.createMu`), para duas criações simultâneas não passarem juntas.
...
3. Se a criação devolver `upload_in_progress` (sessão do próprio usuário para o mesmo destino), o cliente recarrega as pendentes e tenta casar; se não casar, o item falha com essa mensagem. Isso ficou raro: mandar **outro** arquivo com o mesmo nome substitui a sessão parada no servidor ([acima](#criar-sessão)), e sobram os casos em que ela está recebendo bytes agora (outra aba) ou em que o cliente não mandou `mtime`.
