...
6. `StartBackground()` executa imediatamente e depois a cada hora: limpeza de sessões de upload paradas há mais de 24 h, sessões de login expiradas, auditoria com mais de 180 dias e as sobras de jobs interrompidos por um reinício (`job_cleanup`, ver [01](01-visao-geral.md)).
...
| Bytes reservados pelas sessões chunked abertas, por usuário | `FILEZAM_UPLOAD_MAX_RESERVED` (padrão 100 GiB; 0 = sem teto) → 413 `upload_reserve_exceeded` | `Service.insert` |
