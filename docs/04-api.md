...
Settings { slugsEnabled, dropEnabled, dropMaxQuota, dropMaxTtl, dropFileMax, dropMaxFiles, dropMaxLinks, dropStaleAge,
           ...
           mediaEnabled, mediaMaxScan /*entradas percorridas por análise, 100..5 000 000*/,
...
MediaMeta{ kind: "image"|"video"|"audio", format, width?, height? /*já com a rotação aplicada*/, rotation?, durationMs?, codec?, audioCodec?,
           fpsMilli? /*quadros por segundo × 1000: 29970 = 29,97*/, bitrate? /*bits/s, média sobre o arquivo*/, channels?, sampleRate?, bitDepth?,
           takenAt? /*ms; o relógio do equipamento lido como UTC — o formato não grava fuso*/, camera?, lens?, iso?, exposure?, fNumber?, focalLength? }
MediaStats{ files, dirs, bytes, images, videos, audios, others, unreadable,
            imageBytes, videoBytes, audioBytes, otherBytes, videoDuration /*ms*/, audioDuration, pixels,
            videoRes, imageRes, imageMp, imageShape, videoShape, formats, codecs, fps, cameras /*Bucket[]*/,
            longest?, largest?, sharpest? /*{path, name, bytes?, width?, height?, durationMs?}*/, oldest?, newest?, partial }
Bucket   { key /*identificador estável: "4k", "1080p", "portrait", "mp24", "H.264", "3840×2160"; a interface traduz*/, count, bytes?, durationMs? }
```
...
| GET | `/api/files/dirs?path=` | U | `{path, dirs: [string]}` só as subpastas reais de `path` (sem symlinks nem nomes com bytes inválidos), em ordem de leitura do diretório — quem ordena e quem esconde as que começam por ponto é a interface. É o que alimenta o seletor de destino de "Mover para…": a listagem completa traria arquivos que o seletor não usa |
| GET | `/api/files/stat?path=` | U | `{path, entry}` |
| GET | `/api/files/info?path=` | U | Propriedades: `{path, entry, totals?: {files, dirs, bytes, partial}, shares?: Share[], favorite?: bool, media?: MediaMeta}`. `totals`/`shares`/`favorite` só para pastas; a contagem para em 200 000 entradas ou 15 s (`partial: true`). Shares: os do usuário, ou todos se admin. `media` só para arquivos, quando a análise está ligada e o cabeçalho abre (leitura barata: um arquivo, e o resultado fica em cache) |
...
| POST | `/api/files/media/stats` | U | `{paths: [string]}` (1..1000): analisa arquivos e pastas (cada pasta inteira, sem contar repetido quando a seleção se sobrepõe). 200 `{stats: MediaStats, files?: [{path, name, size, meta?}]}` — `files` só com até 2000 arquivos. **202 `{job, pending}`** quando há mais de 250 cabeçalhos ainda não lidos: a leitura corre em segundo plano (job de tipo `media`, sem escrever nada no disco) e a mesma chamada, repetida depois, sai do cache. `stats.partial` quando a varredura bateu em `media_max_scan`. 400 `bad_paths`, 403 `feature_disabled` |
| GET | `/api/files/media/csv?path=` | U | A mesma análise, arquivo a arquivo, como CSV (`attachment`, nunca inline). 409 `media_pending` se ainda houver mais de 250 cabeçalhos por ler — faça a análise antes. 403 `feature_disabled` |
...
| HTTP | code | Quando |
|---|---|---|
| 400 | `bad_json`, `incomplete_body` (a transferência acabou antes dos bytes prometidos), `invalid_path` (inclui tamanho de upload fora de `[0, 1 PiB]` e nomes `.filezam-*`), `invalid_name` (inclui caracteres de controle e os controles bidi U+202A–U+202E/U+2066–U+2069 em nomes novos), `bad_query`, `nested`, `root_op`, `bad_index`, `bad_length`, `bad_conflict`, `bad_expiry`, `bad_id`, `bad_meta`, `bad_multipart`, `no_paths`, `bad_paths`, `too_many`, `too_many_files`, `weak_password`, `bad_archive`, `archive_encrypted`, `invalid_slug`, `password_required`, `bad_mode`, `invalid_username`, `invalid_role`, `invalid_scope`, `bad_quota`, `unsupported` | Entrada inválida |
...
| 409 | `exists`, `is_dir`, `not_dir`, `not_empty`, `modified`, `media_pending`, `slug_taken`, `drop_count_exceeded`, `drop_links_exceeded`, `conflict`, `cross_device`, `upload_in_progress`, `incomplete`, `last_admin`, `self`, `totp_setup_expired`, `totp_not_enabled`, `totp_already_enabled` | Conflito de estado |
