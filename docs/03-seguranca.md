...
- **Link de envio (`mode = drop`, migração 011)** — escrita anônima, com camadas:
  ...
  - **Admissão serializada por link**: contar, somar e escolher o nome acontecem sob o mesmo mutex, senão visitantes simultâneos leriam o mesmo estado e passariam todos. O mutex cobre **só** essas operações de metadados — nunca a transferência dos bytes, que dura o tempo da rede de quem envia. Cobrir a transferência colocava os envios do mesmo link em fila, e o segundo esperava a subida inteira do primeiro: tempo de sobra para um proxy desistir e devolver erro a quem estava enviando. Em troca, a cota pode ser ultrapassada pelo que está em voo — no máximo os envios simultâneos permitidos por IP, cada um limitado ao tamanho de um bloco.
