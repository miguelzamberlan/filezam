...
3. **Nomes novos** (`ValidName`, aplicado por `Mkdir`/`MkdirAll`, rename, PUT, lote e sessões de upload): sem `/`, NUL, `.`/`..`, prefixo reservado, UTF-8 inválido nem **caracteres de controle** (`< 0x20`, `0x7f`). Entradas já existentes com nomes assim continuam legíveis, renomeáveis e apagáveis.
   **Nomes equivalentes** (`NameKey`, `Namer` em `names.go`) contam como o mesmo nome.
   - **Regra**: o Windows e o Samba não diferenciam maiúsculas, e o macOS grava acentos decompostos (NFD). No Linux, inclusive num NTFS montado pelo `ntfs3` sem `nocase`, `C8347.MP4` e `C8347.mp4` convivem, e quem abre a pasta pelo Samba vê dois itens iguais dos quais só um abre. Por isso, em qualquer sistema de arquivos, o Filezam trata como ocupado todo nome que difere de um existente só na caixa ou na forma Unicode.
   - **O que não entra na regra**: acentos continuam contando (`relatório` ≠ `relatorio`), e o nome é gravado como veio: só a comparação é dobrada.
   - **Onde vale**: todo caminho que cria ou renomeia um nome — `Finalize` (PUT, lote, blocos e link de recebimento), `Mkdir`, `MkdirAll`, `Rename`, `MoveTo`, `ResolveDest`, a cópia de árvore, `UniqueName`/`UniqueIfExists` e a extração.
     - Sem substituir, é conflito (`NameTakenError`, 409 `exists` com `existing`).
     - Substituindo, o arquivo novo é gravado sobre o existente, com o nome dele.
     - Pastas intermediárias são reaproveitadas: `fotos/2026` cai dentro de uma `Fotos` existente.
     - Na extração, "Docs/" e "docs/" viram uma pasta só, e a segunda entrada equivalente é pulada.
     - Renomear só a caixa do próprio item é permitido.
   - **Custo e exatidão**: o nome exato é sempre conferido no disco. As variantes vêm de uma leitura da pasta feita uma vez por requisição ou job, e o lote reusa o `Namer` entre os arquivos. Um nome equivalente criado por outro escritor entre essa leitura e a gravação escapa, a mesma janela de qualquer checagem antes de gravar. Duplicatas que já estão no disco continuam listadas e acessíveis.
   - **Canário**: `Lookup` passa pelo `os.Root` como o resto, e o teste com o canário cobre links que escapam.
...
10. **Lixeira**: `.filezam-trash` no root do escopo tem o prefixo reservado, logo nunca aparece em listagem, pesquisa, cópia, zip ou contagem, e não é endereçável por `?path=`. `walk`/`copyDir` pulam qualquer nome reservado. Restaurar volta pelo root base com caminhos base-relativos e só de linhas visíveis (as próprias, ou todas dentro do escopo para admin). A linha no banco é gravada **antes** de mover o item, com `pending=1`, e só vira 0 quando o item chegou inteiro: nunca fica item escondido na lixeira sem linha, fora da cota e invisível. Se o movimento falha no meio de uma cópia entre dispositivos, a origem ainda está inteira, então a lixeira parcial e a linha são desfeitas; se falha depois da cópia, ao remover a origem (`ErrSourceNotRemoved`), a linha fica concluída. Uma queda do processo deixa a linha pendente, e a manutenção (`recoverPendingTrash`, só com linhas anteriores à subida do processo) retoma: origem ainda no lugar → termina a exclusão (copia o que falta pulando o que já chegou inteiro — a cópia só publica arquivos completos — e remove a origem); origem sumiu e item na lixeira → limpa temporários e conclui; nenhum dos dois → apaga a linha, **mas só se a pasta da lixeira existe** (com ela à vista e sem o item, não há nada escondido; sem ela, o disco pode não estar montado, e a linha espera). A retenção não apaga lixeira de linha pendente.
...
- Revogação: troca de senha (própria ou pelo admin), mudança de escopo, de perfil ou desativação apagam as demais sessões do usuário. O admin também encerra uma sessão específica pelo **Painel** (`DELETE /api/admin/sessions/{id}`, auditado como `session.revoke`), menos a própria. O `id` exposto ao admin é o SHA-256 guardado no banco, que não serve como cookie. Exclusão de usuário cascateia. As alterações de usuários pelo admin são serializadas por um mutex no processo, para que a checagem de "último admin" não corra com outra requisição.
...
- **Senha do primeiro administrador**: sem `FILEZAM_ADMIN_PASSWORD`, ela é sorteada (16 caracteres de um alfabeto sem `0/o/1/l/i`, ≈79 bits, amostragem por rejeição) e **escrita no log uma única vez**, no nível `WARN`. É uma troca deliberada: escrever um segredo no log é ruim, e um padrão fixo como `admin/admin` é pior — ele vale desde o instante em que o serviço sobe até alguém entrar pela primeira vez, e essa é a primeira coisa que qualquer varredura tenta. A linha do log envelhece sozinha, porque `must_change_password` obriga a trocar no primeiro acesso. Quem precisa de senha conhecida de antemão (instalação automatizada) define a variável e nada é registrado. `filezam reset-admin` segue a mesma regra.
...
- **Cabeçalhos de cache**: esta é a exceção ao `no-store` que vale para todo o resto do conteúdo —
  inclusive para a imagem original, que sai por `serveFile` e **não** fica no cache do navegador. A
  resposta vai com `private, max-age=300`, sem `immutable`: `private` impede proxy compartilhado, os
  cinco minutos cobrem a navegação (rolar a pasta, entrar numa subpasta e voltar) e depois disso o
  `ETag` revalida com um 304 vazio, sem transferir imagem. A janela é curta de propósito: é conteúdo
  do usuário em disco alheio, e o que o navegador guardou continua lá depois do logout. Guardar por
  sete dias com `immutable` seria mais rápido e foi descartado por isso.
...
- **Nomes de arquivo são validados na criação** (`vfs.ValidName`, que vale para criar, renomear, enviar e extrair): sem barra nem NUL, UTF-8 válido, sem caracteres de controle, sem o prefixo reservado `.filezam-` e **sem os controles bidirecionais fortes** U+202A–U+202E e U+2066–U+2069. Estes últimos reordenam o que vem depois deles na hora de desenhar: `nota‮gpj.exe` aparece na listagem como `notaexe.jpg`, e quem envia por um link público não tem conta nenhuma por trás do nome que escolhe. As marcas de direção U+200E/U+200F e U+061C continuam valendo — elas ajustam um trecho sem reordenar o resto e aparecem em nomes legítimos em árabe e hebraico.
...
- **Link de envio (`mode = drop`, migração 011)** — escrita anônima, com camadas:
  ...
  - **Cota obrigatória na criação**, limitada por `drop_max_quota` e pela cota restante do dono, com `max_file_bytes`, `max_files` e `drop_max_links` links ativos por usuário. O teto de 8 sessões abertas por remetente é repartição, não contenção — a identidade sai para quem pedir, então quem descarta o cookie volta zerado; quem segura o total é `max_files`, que conta as sessões abertas junto com os arquivos já recebidos e não depende de cookie nenhum. As sessões em blocos abertas contam na cota enquanto existem, porque pré-alocam o tamanho declarado (`fallocate`) — é isso, e não a cota do usuário, que limita a reserva: `checkQuota` mede por `IndexSumSize`/`ScanLimited`, e ambos pulam `.filezam-*`. Sessões abandonadas caem em `drop_stale_age` (2 h por padrão, contra 24 h dos uploads autenticados). `FILEZAM_UPLOAD_MAX_RESERVED` continua como rede final, já que a sessão é gravada com o `user_id` do dono.
  - **Admissão serializada por link**: contar e somar acontecem sob um mutex por link, senão visitantes simultâneos leriam o mesmo estado e passariam todos. O mutex cobre **só** metadados — nunca a transferência dos bytes, que dura o tempo da rede de quem envia. Cobrir a transferência colocava os envios do mesmo link em fila, e o segundo esperava a subida inteira do primeiro: tempo de sobra para um proxy desistir e devolver erro a quem estava enviando.

    Até onde o mutex vai depende do caminho. Na **sessão em blocos** ele é segurado da leitura da cota até a linha existir, porque a sessão reserva no disco o tamanho declarado: soltar antes deixaria duas criações simultâneas passarem pela mesma leitura e reservarem o dobro do que o link prometeu. No **envio único** ele é solto assim que a admissão passa, e o nome só é escolhido na publicação, que é metadado — ali a cota pode ser ultrapassada pelo que está em voo, no máximo os envios simultâneos permitidos por IP, cada um limitado ao tamanho de um bloco.
  - **A baixa não depende da conexão de quem enviou.** Publicado o arquivo, o recibo em `share_uploads` é gravado com um contexto sem cancelamento: fechar a aba no instante exato da finalização deixaria o arquivo no disco e fora da cota do link, invisível para o teto e para a lista do remetente, para sempre.
...
- Rate limit por IP, no máximo 2 downloads/zips simultâneos por IP e **4 zips públicos simultâneos no servidor inteiro** (um zip de pasta grande custa CPU e leitura de disco por minutos, e visitantes anônimos trocam de IP à vontade). Acima de 2 GB a interface divide o download em partes (`zip/plan`), então cada zip público dura no máximo o de 2 GB de conteúdo — mas o teto não é imposto: quem chama a API direto ainda pede a pasta inteira de uma vez, contido pelos slots. Medir a pasta (`zip/plan`) ocupa um slot de download do IP. Acima disso a requisição **espera** um slot (até 30 s; desiste se o visitante cancelar) em vez de responder 429 na hora: um preview de Markdown pede todas as imagens de uma vez e um `<img>` recusado fica quebrado. Quem espera não ocupa disco nem banda; o número de esperas por IP é limitado pelo rate limit de 120 requisições/min.
...
- **Excluir, mover ou renomear pelo app revoga o link** do item e de tudo abaixo dele, de qualquer dono (`DeleteSharesUnder`, comparação por bytes do caminho, então `pub2` não cai junto com `pub`). A exclusão revoga antes de apagar; restaurar da lixeira devolve o item, não o link. Isso vale em qualquer sistema de arquivos, inclusive nos que reutilizam números de inode. Mudanças por fora não passam por aí: a manutenção horária (`sweepBrokenShares`) confere cada link vivo — item ausente, de outro tipo ou com `dev`/`ino` diferente — e grava `broken_since` na primeira vez, limpa se o item voltar e revoga (`DropShare`, auditado como `share.revoke.broken`) depois de 24 h quebrado. Se a raiz não responde, ou se pelo menos 4 links e mais da metade do total quebram na mesma rodada, nada é revogado: é o retrato de um disco não montado, não de pessoas apagando coisas. O dono do link revogado recebe uma notificação. Antes de agir, a interface consulta `POST /api/shares/affected` e pede confirmação listando os links que cairiam; a resposta conta links de qualquer dono (todos caem), mas só devolve o caminho relativo ao escopo de quem pergunta, o tipo e o apelido — nunca token nem quem criou.
