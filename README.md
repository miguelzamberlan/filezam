...
FILEZAM_TRUSTED_PROXIES=172.31.250.1       # gateway da rede do compose: por onde o proxy do host chega
...
**Só na rede local, sem proxy**: `FILEZAM_BIND=0.0.0.0`, `FILEZAM_TRUSTED_PROXIES=` (vazio) e `FILEZAM_SECURE_COOKIES=false`; acesse `http://ip-do-servidor:8080`. Nunca coloque a faixa da rede local (`192.168.0.0/16`) nem `0.0.0.0/0` em `FILEZAM_TRUSTED_PROXIES`: qualquer máquina passaria a escolher o próprio IP e escaparia do bloqueio de login. Gravidade e combinações (proxy + porta local, proxy em container) em [docs/08](docs/08-operacao.md#porta-local-e-proxies-confiáveis).
...
O que já se sabe que falta (retomada de operações interrompidas, passkeys) e o que foi descartado de propósito (pesquisa por conteúdo, retomada automática de upload) está em [`docs/10-roadmap.md`](docs/10-roadmap.md). Sugestões passam por issue antes de virar código.

## Versões

A versão atual é a **1.2.0**; o histórico está no [`CHANGELOG.md`](CHANGELOG.md). O projeto usa versionamento semântico: versões publicadas não mudam, e correções e novidades saem em versões novas. Para produção, fixe uma versão (`ghcr.io/miguelzamberlan/filezam:1.2.0`, ou `:1.2` para receber só correções) em vez de acompanhar a `main`. Detalhes em [`CONTRIBUTING.md`](CONTRIBUTING.md#versões-e-lançamentos).
