...
- **A new name is taken when an equivalent one exists** (`vfs.NameKey`: case and Unicode form, not accents). Anything that creates or renames an entry checks through `Namer.Lookup`/`Root.Lookup` (or a method that already does: `Finalize`, `Mkdir`, `MkdirAll`, `Rename`, `MoveTo`, `ResolveDest`, `UniqueName`), never `Exists` alone; overwrite writes onto the existing name. Reuse one `Namer` per request or job so a folder is read once, not once per file.
...
- Handlers: `handlers_{auth,admin,files,uploads,jobs,favorites,shares,public,drop,archive,thumbs,media}.go`, `notifications.go` (per-user notices: server stores `kind` + JSON `data`, the UI builds the text; `Notify` groups by `group_key` while unread); SPA in `spa.go`.
...
- Media metadata: `internal/media/` parses container headers only (ISO base media, EBML, RIFF, TIFF/EXIF) and never decodes a frame; every declared length is checked against the file before it is read and never sizes an allocation. Readings are cached in `media_meta` keyed by path + size + mtime; `internal/server/handlers_media.go` falls back to a background job above `mediaSyncMax` unread headers.
