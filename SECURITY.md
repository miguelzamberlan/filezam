...
|---|---|
| 1.x (última tag publicada) | sim |
| `main` (em desenvolvimento) | sim, antes de virar versão |
| anteriores a 1.0.0 | não existem versões públicas |

As correções saem como uma versão `PATCH` nova (1.0.1, 1.0.2…); versões publicadas não são alteradas. Atualize para a última da série.
