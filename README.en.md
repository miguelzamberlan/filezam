...
The current version is **1.1.0**; the history is in [`CHANGELOG.md`](CHANGELOG.md) *(Portuguese)*. The project uses semantic versioning: published versions never change, and fixes and features ship as new versions. In production, pin a version (`ghcr.io/miguelzamberlan/filezam:1.1.0`, or `:1.1` to receive only fixes) instead of following `main`. Details in [`CONTRIBUTING.md`](CONTRIBUTING.md#versões-e-lançamentos) *(Portuguese)*.
